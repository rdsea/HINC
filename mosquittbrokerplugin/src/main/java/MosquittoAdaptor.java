...
public class MosquittoAdaptor  implements ProviderQueryAdaptor<Broker> {
    ...
    public static void main(String[] args){
        MosquittoAdaptor adaptor = new MosquittoAdaptor();
        Map<String, String> settings = new HashMap<>();
        settings.put("endpoint", "http://localhost:3002");

        Collection<Broker> brokers = adaptor.getItems(settings);

        ResourcesProvider provider = adaptor.getProviderAPI(settings);

        ControlPoint controlPoint = new ControlPoint();
        controlPoint.setReference("http://localhost:3002/mosquittobroker");


        System.out.println(adaptor.sendControl(controlPoint).getOutput());
    }
