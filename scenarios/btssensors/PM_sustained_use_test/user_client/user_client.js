...
function provisionSlice(slice){
    ...
    return writeFile(`/tmp/slice${timestamp}.json`, JSON.stringify(slice, null, 4)).then(() => {
        return new Promise((resolve, reject) => {
            // a wait between 1 and 10 seconds
            let wait = Math.floor(Math.random()*(15)+1);
        ...
        return new Promise((resolve, reject) => {
            // a wait between 5 and 15 seconds
            let wait = Math.floor(Math.random()*(15-5+1)+5);
            setTimeout(resolve(), wait*1000);
        })
    }).then(() => {
        // delete slice to clean up
        return spawn(`pizza slice delete slice${timestamp}`);
    }).then(() => {
        return new Promise((resolve, reject) => {
